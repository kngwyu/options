...
class GridWorld:
    ...
    def _fillAdjacencyMatrix(self):
        ...
        for i in range(len(self.matrixMDP)):
            for j in range(len(self.matrixMDP[i])):
                if (
                    i != 0
                    and i != (self.numRows - 1)
                    and j != 0
                    and j != (self.numCols - 1)
                ):
                    if self.matrixMDP[i + 1][j] != -1:
                        self.adjMatrix[self.idxMatrix[i][j]][
                            self.idxMatrix[i + 1][j]
                        ] = 1
                    if self.matrixMDP[i - 1][j] != -1:
                        self.adjMatrix[self.idxMatrix[i][j]][
                            self.idxMatrix[i - 1][j]
    ...
    def getNextStateAndRewardFromOption(self, currState, o_pi, actionSet):
        ...
        if currState == goalIdx:
            ...
            self.useNegativeRewards
            and nextAction == aTerminate
            and self.matrixMDP[self.currX][self.currY] != -1
        ):
            accum_reward = -1
        elif (
            not self.useNegativeRewards
            and nextAction == aTerminate
            and self.matrixMDP[self.currX][self.currY] != -1
        ):
            accum_reward = 0
