...
def optionDiscoveryThroughPVFs(env, epsilon, verbose, discoverNegation):
    ...
    options, actionSetPerOption = discoverOptions(
        env,
        epsilon=epsilon,
        verbose=verbose,
        discoverNegation=discoverNegation,
        plotGraphs=True,
    )
...
def getExpectedNumberOfStepsFromOption(
    env, eps, verbose, discoverNegation, loadedOptions=None
):
    ...
    if loadedOptions is None:
        if verbose:
            options, actionSetPerOption = discoverOptions(
                env, eps, verbose, discoverNegation, plotGraphs=True
            )
        else:
            options, actionSetPerOption = discoverOptions(
                env, eps, verbose, discoverNegation, plotGraphs=False
            )
    else:
    ...
    if discoverNegation:
        for i in range(numOptions // 2):
            listToPrint = stats.getAvgNumStepsBetweenEveryPoint(
                actionSet,
                actionSetPerOption,
                verbose,
                initOption=i * 2,
...
def qLearningWithOptions(
    ...
    if loadedOptions is None:
        if verbose:
            options, actionSetPerOption = discoverOptions(
                env, options_eps, verbose, useNegation, plotGraphs=True
            )
        else:
            options, actionSetPerOption = discoverOptions(
                env, options_eps, verbose, useNegation, plotGraphs=False
            )
    else:
        options = loadedOptions
        actionSetPerOption = []
    ...
    while (
        i < len(genericNumOptionsToEvaluate)
        ...
        for s in range(numSeeds):
            ...
            for i in range(numEpisodes):
                ...
                returns_eval[idx][s].append(
                    learner.evaluateOneEpisode(eps=0.01, timestepLimit=maxLengthEp)
                )
    ...
    for s in range(numSeeds):
        ...
        learner = QLearning(
            alpha=alpha, gamma=gamma, epsilon=epsilon, environment=env, seed=s
        )
        for i in range(numEpisodes):
...
if __name__ == "__main__":
    ...
    if taskToPerform == 1:  # Discover options
        optionDiscoveryThroughPVFs(
            env=env, epsilon=epsilon, verbose=verbose, discoverNegation=bothDirections
        )
        ...
        taskToPerform == 4
    ):  # Compute the average number of time steps between any two states
        gamma = 1.0
        ...
        getExpectedNumberOfStepsFromOption(
            env=env,
            eps=epsilon,
            verbose=verbose,
        ...
        for i in range(num_episodes):
            ...
            returns_eval.append(
                learner.evaluateOneEpisode(eps=0.01, timestepLimit=max_length_episode)
            )
        ...
        taskToPerform == 6
    ):  # Solve for a given goal w/ primitive actions (q-learning) following options
        ...
        plt.plot(
            Utils.movingAverage(average),
            label="Prim. act.",
            color=Utils.colors[color_idx],
        )
        plt.fill_between(
            range(len(Utils.movingAverage(average))),
        ...
        for idx, numOptionsToUse in enumerate(totalOptionsToUse):
            ...
            minConfInt, maxConfInt = Utils.computeConfInterval(
                average, std_dev, num_seeds
            )
        ...
        (
            returnsEvalPrimitive1,
            returnsEvalDiscovered,
            totalOptionsToUseDiscovered,
        ) = qLearningWithOptions(
            env=env,
            alpha=0.1,
            gamma=0.9,
            options_eps=0.0,
        ...
        (
            returnsEvalPrimitive2,
            returnsEvalLoaded,
            totalOptionsToUseLoaded,
        ) = qLearningWithOptions(
            env=env,
            alpha=0.1,
            gamma=0.9,
        ...
        for idx, numOptionsToUse in enumerate(totalOptionsToUseDiscovered):
            if numOptionsToUse == numOptionsDiscoveredToConsider:
                ...
                plt.plot(
                    Utils.movingAverage(average),
                    label=str(numOptionsToUse) + " opt. disc.",
                    color=Utils.colors[color_idx],
                )
        ...
        for idx, numOptionsToUse in enumerate(totalOptionsToUseLoaded):
            if numOptionsToUse == numOptionsLoadedToConsider:
                ...
                plt.plot(
                    Utils.movingAverage(average),
                    label=str(numOptionsToUse) + " opt. loaded",
                    color=Utils.colors[color_idx],
                )
